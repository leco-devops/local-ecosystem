...
NETWORK_CONTAINERS="traefik open-webui ollama airllm n8n_postgres n8n paperclip_postgres paperclip service-dashboard leco-update-catalog minio valkey r2-adapter kv-adapter d1-adapter browser-rendering-local workers-runtime autoscaler autoscale-demo mysql redis mailpit telegram-gateway cache-nginx cache-varnish adminer redis-commander leco-sftp leco-ftp leco-file-browser"
START_ORDER="traefik postgres ollama airllm webui n8n paperclip-postgres paperclip dashboard update-catalog cloudflare-local infra file-transfer"
