"""
AirLLM: pinned file, /api/tags + /api/ps + /api/version, full model management.
Mirrors ollama_models.py for the AirLLM Ollama-compatible shim.
"""
...
PINNED_MODELS_FILE = Path(PROJECT_ROOT) / "ecosystem-stack" / "config" / "airllm-pinned-models.txt"
AIRLLM_BASE = os.getenv("AIRLLM_BASE_URL", "http://airllm:11435").rstrip("/")
BACKUP_DIR = os.path.join(PROJECT_ROOT, ".local-eco-backups")
os.makedirs(BACKUP_DIR, mode=0o755, exist_ok=True)

_PINNED_HEADER = """# AirLLM Pinned Models
# Models listed here are automatically pulled when the AirLLM shim starts.
# One HuggingFace model ID per line; lines starting with # are ignored.
#
# IMPORTANT: AirLLM uses HuggingFace safetensors format, NOT Ollama GGUF.
# These models are separate from your Ollama models.
#
...
#
# Large models (AirLLM's specialty - can run on limited VRAM):
# Qwen/Qwen2.5-72B-Instruct
# meta-llama/Meta-Llama-3.1-70B-Instruct  # Requires HF_TOKEN for gated access
#
"""
...
def _airllm_get(path: str, timeout=12):
    try:
        r = requests.get(f"{AIRLLM_BASE}{path}", timeout=timeout)
        return r.status_code, r.json() if r.content else {}
    except Exception as exc:
        return None, {"error": str(exc)}


def _airllm_post(path: str, payload: dict, timeout=120):
    try:
        r = requests.post(f"{AIRLLM_BASE}{path}", json=payload, timeout=timeout)
...
def _airllm_delete(path: str, payload: dict, timeout=120):
    try:
        r = requests.delete(f"{AIRLLM_BASE}{path}", json=payload, timeout=timeout)
        if r.status_code == 200:
            return True, r.json() if r.content else {}
        try:
            err = r.json()
...
def _airllm_version() -> dict | None:
    code, data = _airllm_get("/api/version", timeout=6)
    if code == 200 and isinstance(data, dict):
        return data
    return None


def _airllm_tags() -> tuple[bool, list[dict]]:
    code, data = _airllm_get("/api/tags")
...
def _airllm_ps() -> tuple[bool, list[dict]]:
    code, data = _airllm_get("/api/ps")
    if code != 200 or not isinstance(data, dict):
        return False, []
    models = data.get("models") or []
    return True, models if isinstance(models, list) else []
...
def canonical_model_name(name: str) -> str:
    """Stable key: use the full HF repo ID as-is for AirLLM."""
    s = (name or "").strip()
    return s
...
def build_models_payload() -> dict:
    ...
    ok_tags, installed = _airllm_tags()
    ok_ps, running = _airllm_ps()
    reachable = ok_tags or ok_ps
    version = _airllm_version() if reachable else None
    ...
    for m in installed:
        ...
        by_name[n] = {
            ...
            "installed": True,
            "details": details,
            "airllm_ready": m.get("airllm_ready", False),
        }
    ...
    def merged_row_for_canonical(canon: str) -> dict:
        ...
        return {
            ...
            "airllm_ready": base.get("airllm_ready", False),
        }
    ...
    return {
        "ok": True,
        "airllm_reachable": reachable,
        "airllm_base": AIRLLM_BASE,
...
def airllm_inspect(model: str) -> dict:
    model = (model or "").strip()
    if not model:
        return {"ok": False, "error": "model name required"}
    ok, data = _airllm_post("/api/show", {"name": model}, timeout=60)
    if ok:
...
def _pull_bg(model: str):
    try:
        with requests.post(
            f"{AIRLLM_BASE}/api/pull",
            json={"name": model.strip(), "stream": False},
            timeout=(15, 7200),
        ) as r:
...
def airllm_delete(model: str) -> dict:
    model = (model or "").strip()
    if not model:
        return {"ok": False, "error": "model name required"}
    ok, data = _airllm_delete("/api/delete", {"name": model})
    if ok:
        return {"ok": True, "model": model}
    return {"ok": False, "error": data, "model": model}


def airllm_unload(model: str) -> dict:
    model = (model or "").strip()
    if not model:
        return {"ok": False, "error": "model name required"}
    try:
        r = requests.post(
            f"{AIRLLM_BASE}/api/generate",
...
def airllm_warm(model: str, keep_alive: str = "-1") -> dict:
...
def airllm_unload_all() -> dict:
    ok, running = _airllm_ps()
    if not ok:
...
def backup_manifest() -> dict:
    ...
    fname = f"airllm-manifest-{ts}.json"
...
def list_manifest_backups() -> dict:
    try:
        names = sorted(
            [f for f in os.listdir(BACKUP_DIR) if f.startswith("airllm-manifest-") and f.endswith(".json")],
            reverse=True,
        )[:80]
        items = []
        for n in names:
...
def restore_manifest(filename: str) -> dict:
    ...
    if not (base.startswith("airllm-manifest-") and base.endswith(".json")):
        return {"ok": False, "error": "invalid backup filename"}
...
def handle_models_action(request, data: dict | None) -> tuple[dict, int]:
    ...
    if action == "pull_all":
        ...
        return body, 200 if body.get("ok") else 400
    if action == "delete":
        body = airllm_delete(model)
        return body, 200 if body.get("ok") else 400
    if action in ("unload", "off"):
        body = airllm_unload(model)
        return body, 200 if body.get("ok") else 400
    if action in ("on", "reinstall", "ensure"):
        body = start_pull(model)
        return body, 200 if body.get("ok") else 400
    if action in ("warm", "load", "load_ram"):
        body = airllm_warm(model, keep_alive=keep_alive)
        return body, 200 if body.get("ok") else 400
    if action == "unload_all":
        body = airllm_unload_all()
        return body, 200 if body.get("ok") else 400
