...
|--------|----------------|------------------------------|
| **503** “Backend fetch failed” (Varnish page) right after restart | **`server`** not listening yet (apt-get/npm) or crash loop (`sudo varnishncsa` in server container); **`varnish`** started before Express | Use **`sample-node-varnish-multiprocess`** (or AI onboarding): **`server` healthcheck**, **`varnish` `depends_on: server: service_healthy`**, **`LECO_DISABLE_VARNISH_NCSA=true`**, VCL backend **`<slug>-server:3000`**. See **`docs/help/09-503-varnish-backend.md`**. |
