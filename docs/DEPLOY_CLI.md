...
- **`additionalComposeFilesFromManifest`** — optional list merged **after** the primary compose file (whether that primary is **`composeFile`** or **`composeFileFromManifest`**); paths are relative to **`leco.app.yaml`’s directory** (e.g. `hosting/app-available/myapp/docker-compose.leco-hosting.yml`). Keeps Traefik **`lh-network`** joins and public URL env overrides in the ecosystem repo while **`composeFile`** still points at the upstream project when you are **not** using **`composeFileFromManifest`**. Sample: **`hosting/samples/sample-leco-hosting-overlay/`**. For **split UI + `/api` routes**, frontends should call **`https://<slug>.lh/api/...`** (same origin), not `localhost:PORT` baked from upstream compose — the sample overlay sets **`REACT_APP_BACKEND_URL: ""`** and **`REACT_APP_SITE_URL`** (or your stack’s equivalent) so the browser uses the Traefik hostname.
