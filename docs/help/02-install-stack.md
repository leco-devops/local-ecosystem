...
Traefik → Postgres → Ollama → **AirLLM** → Open WebUI → n8n → **Paperclip** (postgres + app) → **service-dashboard** → cloudflare-local → infra compose → **file-transfer** (when enabled in platform config or full profile).
