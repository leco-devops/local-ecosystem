...
Use this pattern when the **application repository** should stay free of LEco-specific Docker edits (Traefik `lh-network`, `*.lh` URLs, CORS, host port stripping), while **local-ecosystem** still drives deploy from `hosting/app-available/<slug>/`.
...
4. In the hosting overlay, use **`ports: !reset []`** on any upstream service that publishes host ports. This keeps the stack reachable only through Traefik / Docker networking and avoids collisions on **`:80`**, **`:3000`**, **`:5432`**, etc.
...
- Add **`ports: !reset []`** for every upstream service that published host ports. LEco DevOps now auto-generates this for new hosting overlays during onboarding.
...
- **`hosting/samples/sample-hosting-compose-entry/`** — use **`composeFileFromManifest`** + **`include`** when you want the hosting tree to own the **primary** compose file, not just an extra overlay.
