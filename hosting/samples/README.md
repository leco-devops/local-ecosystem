...
| **`sample-node-varnish-multiprocess/`** | Multi-process Node.js + Varnish HTTP cache + MongoDB + Redis. Full 7-file template: runtime config preloader (`leco-docker-preload.js`), `conf/varnish/` custom service config, hosting overlay (`LECO_DISABLE_VARNISH_NCSA`, server healthcheck, varnish waits for healthy server), shared node_modules volume. See [09-503-varnish-backend.md](../../docs/help/09-503-varnish-backend.md). |
...
| **`sample-cf-multi-wrangler-monorepo/`** | Several Workers under `infra/wrangler.*.toml` plus **`wrangler.pages.toml`**: full `runtimes[]`, `configRefs`, and routing (`/api` → api, `/` → dashboard). Documents automatic config symlinks on materialize. |
