...
- **Connect any AI agent — per-agent setup:** [`docs/CONNECT_AI_AGENTS.md`](docs/CONNECT_AI_AGENTS.md) and a **Set up a specific agent** picker in **MCP → 2 · Install on an agent**, covering Claude Code, Claude Desktop and Cowork (one config file serves both), Codex CLI, Antigravity, and the generic shape for Cursor / VS Code / Windsurf / Cline / Zed / Continue / JetBrains / Gemini CLI. The snippets are generated with this machine's **absolute** host paths, because a GUI agent does not inherit the shell `PATH` — a bare `leco-mcp` resolves in a terminal and then fails silently inside the app, which is the most common setup failure. Guidance leads with the HTTP transport (`http://localhost:8099/mcp`), which needs nothing installed since the container already serves it. Clients exercised on a real machine are marked `verified`; the rest say plainly that they were not. Help page [`docs/help/22-connect-ai-agents.md`](docs/help/22-connect-ai-agents.md).
...
- **The MCP server reported an address nobody routes on.** Every tool answered from `http://localhost:8090` — the published host port, an implementation detail of the compose publish, not where the dashboard lives. Two independent causes: `localhost:8090` sat first in the candidate list in [`tools/mcp-server/leco_mcp/config.py`](tools/mcp-server/leco_mcp/config.py), and both `.mcp.json` registrations pinned `LECO_MCP_DASHBOARD_URL` to it, so the explicit value won before discovery ever ran. The routed hostname is now tried first and the domain is **read, not assumed** — `routing_domain()` mirrors `dashboard/leco_detect.py` exactly, so a local install resolves `http://localhost.lh` (then the `dashboard.lh` alias) while a cloud install resolves `https://dashboard.<base_domain>` from `config/leco-platform.yaml`; `LECO_MCP_BASE_DOMAIN` overrides, and an unreadable or malformed config falls back to `.lh` rather than emitting an address nothing answers on. HTTP is tried before HTTPS on `.lh` because httpx verifies against certifi, which does not trust the mkcert CA. `localhost:<DASHBOARD_HOST_PORT>` is kept immediately behind the routed names rather than dropped — Traefik being down is precisely when these tools are needed — which makes the resolved address a diagnostic in itself: `leco_server_info` reporting the host port now means `*.lh` routing is broken while the dashboard is healthy.
- **Plugin install instructions could not work as written.** Two independent faults, both reported from a real attempt. `claude plugin marketplace add ./` resolves the path against the shell's working directory, so it fails everywhere except this repo's root — and the moment you actually run it is while standing in the application you are onboarding; the dashboard and docs now emit the **absolute** path. Separately, `claude plugin marketplace add leco-devops/local-ecosystem` clones the repository's **default branch**, and the plugin has never been merged to `main`, so the clone succeeds with no marketplace file and reports `Marketplace file not found at …` — which reads as a broken install rather than a branch that does not carry the plugin. The GitHub form now carries that caveat in the UI and the docs, with a one-line `curl` to check the branch. `marketplace add` has no `--branch` flag.

- **Certificate SANs could be harvested from application data.** `certs/generate-certs.sh` discovered hostnames with a recursive `grep` over `hosting/app-available/`, which also holds seeded application data — 3.1 GB of MongoDB BSON in one real slot here. Every `http://x.lh`-shaped byte sequence in those dumps became a SAN on the certificate this machine **trusts**, so captured HTTP traffic in a log could reach the local trust store. Discovery now prunes `data`, `node_modules`, `.git`, build output and virtualenvs. Hostnames come from configuration, never from an application's data. Side effect: `--list` went from ~100 s to ~0.35 s (BSD `grep -r` on macOS also follows symlinks, so a `source` link dragged in an app's whole checkout), the SAN list is byte-identical, and the dashboard test suite dropped from 142 s to 19 s — the 120 s-timeout cert test had been failing whenever the machine was busy.
- **MCP container served a stale tool surface.** The running `leco-mcp` image predated the current tools, so agents connecting over HTTP saw 60 tools and no `leco_verify`, while the dashboard reported the source count. Rebuilt; `./ecosystem-stack/services/mcp.sh restart` is the fix when the counts disagree, and the symptom is now listed in the troubleshooting table.
- **Agent picker reset itself every few seconds.** The MCP tab re-renders on the dashboard refresh interval, which threw away the selected agent mid-read. The selection now survives a re-render.
